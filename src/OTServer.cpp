...
// TODO: da2ce7 put a bunch of hardcoded numbers in here. These need to be
// changed
...
bool OTServer::LoadConfigFile()
{
...

    {
        ...
        if (NULL == szValue)
            p_Config->CheckSet_str("permissions", "override_nym_id", NULL,
                                   strValue, bIsNewKey);
        else
    ...
    , m_lTransactionNumber(0)
{
    ...
    // caller will shutdown gracefully.
    // (Caller must regularly check the flag and shutdown when it sees the
    // change.)
}
...
{
    if (NULL == m_pServerContract) delete m_pServerContract;
    m_pServerContract = NULL;
...
void OTServer::Init(bool bReadOnly /*=false*/)
{
    ...
    // OTDB_DEFAULT_PACKER,SERVER_MAIN_FILENAME);
        ...
        while (xml && xml->read()) {
            ...
            case irr::io::EXN_ELEMENT: {
                ...
                /*
                  ...
                  OT_ASSERT_MSG(NULL != pContract, "Error allocating memory for
...
void OTServer::UserCmdGetMarketList(OTPseudonym&, OTMessage& MsgIn,
                                    OTMessage& msgOut)
{
...
void OTServer::UserCmdGetMarketOffers(OTPseudonym&, OTMessage& MsgIn,
                                      OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if ((msgOut.m_bSuccess =
             ((pMarket != NULL) ? true : false))) // if assigned true
    {
        OTASCIIArmor ascOutput;
...
void OTServer::UserCmdGetMarketRecentTrades(OTPseudonym&, OTMessage& MsgIn,
                                            OTMessage& msgOut)
{
    // (1) set up member variables
    msgOut.m_strCommand =
...
//
void OTServer::UserCmdGetNym_MarketOffers(OTPseudonym& theNym, OTMessage& MsgIn,
                                          OTMessage& msgOut)
{
    // (1) set up member variables
...
void OTServer::UserCmdCheckServerID(OTPseudonym&, OTMessage& MsgIn,
                                    OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (MsgIn.m_strServerID ==
        m_strServerID) // ServerID, a hash of the server contract.
...
void OTServer::UserCmdGetTransactionNum(OTPseudonym& theNym, OTMessage& MsgIn,
                                        OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if ((bGotNymboxHashServerSide && bGotNymboxHashClientSide &&
        ...
        OTLog::Output(0, "OTServer::UserCmdGetTransactionNum: Rejecting "
                            ...
                            // out at a single time.)
    {
        OTLog::vOutput(
            0,
            "OTServer::UserCmdGetTransactionNum: Failure: Nym %s already has "
                                          ...
                                          // time now.)
        {
            ...
            {
                lTransNum = 0;
                OTLog::Error("OTServer::UserCmdGetTransactionNu: Error issuing "
        ...
        if (false == bSuccess) {
            ...
            OTLog::Error(
                "Error loading Nymbox in OTServer::UserCmdGetTransactionNum\n");
        }
        // Drop in the Nymbox
        else if ((msgOut.m_bSuccess = (
                      //                                         theLedger.VerifyAccount(m_nymServer)
                      ...
                      theLedger.VerifySignature(
                          m_nymServer) // everything else without loading them.
                  ...
                  ))                   // if success
        {
            ...
            if (NULL != pTransaction) // The above has an OT_ASSERT within, but
            ...
            OTLog::Error("Error verifying Nymbox in "
                         "OTServer::UserCmdGetTransactionNum\n");
        }
...
void OTServer::UserCmdGetRequest(OTPseudonym& theNym, OTMessage& MsgIn,
                                 OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (!msgOut.m_bSuccess) {
        OTLog::Error(
            "Error loading request number in OTServer::UserCmdGetRequest\n");
    }
    else
...
void OTServer::UserCmdSendUserMessage(OTPseudonym& theNym, OTMessage& MsgIn,
                                      OTMessage& msgOut)
{
    // (1) set up member variables
...
void OTServer::UserCmdSendUserInstrument(OTPseudonym& theNym, OTMessage& MsgIn,
                                         OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    const char* szCommand /*=NULL*/)
{
    OT_ASSERT_MSG(
        ...
        "pMsg and pPayment -- these can't BOTH be NULL.\n"); // must provide one
                                                             // or the other.
    OT_ASSERT_MSG(
        !((NULL != pMsg) && (NULL != pPayment)),
        "pMsg and pPayment -- these can't BOTH be not-NULL.\n"); // can't
                                                                 // provide
                                                                 // both.
    OT_ASSERT_MSG((NULL == pPayment) ||
                      ((NULL != pPayment) && pPayment->IsValid()),
    ...
    // NULL in the case where payment isn't NULL)
...
// Therefore in that case, NULL would be passed for pMsg, meaning that inside
...
// open a message, so we must create one to give him. So if pMsg is NULL, then
// this function will
...
// Of course, if pMsg was not NULL, that means the message (and instrument
// inside of it) already exist,
// so no instrument would need to be passed. But if pMsg IS NULL, that means the
...
//                (And since the instrument defaults to NULL, this makes pMsg
// the final argument in the call.)
//         but if pMsg is NULL, then you must pass the payment instrument as the
                                     ...
                                     // replace pMsg->m_strCommand below
{
    OT_ASSERT_MSG(
        ...
        "pMsg and pstrMessage -- these can't BOTH be NULL.\n"); // must provide
                                                                // one or the
                                                                // other.
    OT_ASSERT_MSG(
        !((NULL != pMsg) && (NULL != pstrMessage)),
        "pMsg and pstrMessage -- these can't BOTH be not-NULL.\n"); // can't
                                                                    // provide
                                                                    // both.
    ...
    //  else // pMsg was passed in, so it's not NULL. No need to create it
    // ourselves like above. (pstrMessage should be NULL anyway in this case.)
    //  {
    ...
    if ((theLedger.LoadNymbox() && // I think this loads the box receipts too,
        ...
        if (NULL != pTransaction) // The above has an OT_ASSERT within, but I
...
void OTServer::UserCmdCheckUser(OTPseudonym&, OTMessage& MsgIn,
                                OTMessage& msgOut)
{
    // (1) set up member variables
...
*/
void OTServer::UserCmdUsageCredits(OTPseudonym& theNym, OTMessage& MsgIn,
                                   OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (bSuccessLoadingNymbox)
        bSuccessLoadingNymbox = (theNymbox.VerifyContractID() &&
                                 theNymbox.VerifyAccount(m_nymServer));
    else {
    ...
    if (!bErrorCondition) {
        ...
        if (0 != lAdjustment) {
            ...
            msgOut.m_bSuccess = pNym->SaveSignedNymfile(
                m_nymServer); // We changed it, so let's save pNym...
        }
        else
...
void OTServer::UserCmdIssueAssetType(OTPseudonym& theNym, OTMessage& MsgIn,
                                     OTMessage& msgOut)
{
        ...
        else // success instantiating contract.
        {
            ...
            else // success loading contract from string.
            {
                       ...
                       // contract.
                {
                    ...
                    // public key in OTServer::UserCmdIssueAssetType.\n",
                    //                                      szFunc);
                    //                    }
                    ...
                    // OTServer::UserCmdIssueAssetType.\n",
                    //                                          szFunc);
                    //                        }
                    //                    }
                }
                ...
                // are one.
            {
                if (pAssetContract->VerifyContract()) {
                    ...
                    pNewAccount = OTAccount::GenerateNewAccount(
                        USER_ID, SERVER_ID, m_nymServer, MsgIn,
                        OTAccount::issuer);

                    // If we successfully create the account, then bundle it in
                    // the message XML payload
                    if (NULL != pNewAccount) // This last parameter generates an
                                             // ISSUER account
                    {                        // instead of the default SIMPLE.
                        ...
                        AddAssetContract(*pAssetContract); // Do NOT clean this
                                                           // up unless failure!
                                                           // Server will clean
                                                           // it up.
                        ...
                        if (true ==
                            ...
                            bSuccessLoadingInbox = theInbox.VerifyAccount(
                                m_nymServer); // todo -- this should NEVER
                        ...
                        if (true ==
                            ...
                            if (bSuccessLoadingOutbox) {
                                bSuccessLoadingOutbox =
                                    theOutbox.SignContract(m_nymServer);
...
void OTServer::UserCmdIssueBasket(OTPseudonym& theNym, OTMessage& MsgIn,
                                  OTMessage& msgOut)
{
    // (1) set up member variables
            ...
            if (bSubCurrenciesAllExist) {
                ...
                for (int32_t i = 0; i < theBasket.Count(); i++) {
                    ...
                    */
                    ...
                    pNewAccount = OTAccount::GenerateNewAccount(
                        SERVER_USER_ID, SERVER_ID, m_nymServer, MsgIn,
                        OTAccount::basketsub);
                                    ...
                                    OTMessage& msgOut)
{
    ...
    OTAccount* pNewAccount =
        OTAccount::GenerateNewAccount(USER_ID, SERVER_ID, m_nymServer, MsgIn);
    ...
    if (NULL != pNewAccount) {
        ...
        if (true == bSuccessLoadingInbox) // WEIRD IF THIS HAPPENED...
            bSuccessLoadingInbox = theInbox.VerifyAccount(
                m_nymServer); // todo -- this should NEVER happen, the ID was
...
void OTServer::NotarizeTransfer(OTPseudonym& theNym, OTAccount& theFromAccount,
                                ...
                                bool& bOutSuccess)
{
    ...
    if (false == NYM_IS_ALLOWED(strUserID.Get(), __transact_transfer)) {
        ...
        OTLog::vOutput(0, "OTServer::NotarizeTransfer: Expected "
    ...
    // So we treat it that way... I either get it successfully or not.
        ...
        OTString strTemp(tranIn);
        OTLog::vOutput(
            0, "OTServer::NotarizeTransfer: Failed attempt by user %s in "
        ...
        OTCleanup<OTAccount> theDestAcctGuardian(
            pDestinationAcct); // This is safe in cases where NULL is returned.
                               // No more need to cleanup pDestAcct.

        // Only accept transfers with positive amounts.
        if (0 > pItem->GetAmount()) {
            OTLog::Output(0, "OTServer::NotarizeTransfer: Failure: Attempt to "
        ...
        // does not match the "Acct From" ID on this transaction item
            ...
            OTLog::Output(0, "OTServer::NotarizeTransfer: Error: 'From' "
        ...
        // ok so the IDs match. Does the destination account exist?
            ...
            OTLog::Output(0, "OTServer::NotarizeTransfer: ERROR verifying "
        ...
        //
            ...
            OTLog::Output(0, "OTServer::NotarizeTransfer: Failure: Destination "
        ...
        // performing the actual transfer.
            ...
            if (true == bSuccessLoadingInbox)
                ...
                OTLog::Error(
                    "OTServer::NotarizeTransfer: Error loading 'to' inbox.\n");
            //            else
            //                bSuccessLoadingInbox    =
            // theToInbox.GenerateLedger(pItem->GetDestinationAcctID(),
            // SERVER_ID, OTLedger::inbox, true); // bGenerateFile=true
            if (true == bSuccessLoadingOutbox)
                bSuccessLoadingOutbox =
                    theFromOutbox.VerifyAccount(m_nymServer);
            else
                OTLog::Error("OTServer::NotarizeTransfer: Error loading 'from' "
                             "outbox.\n");
            //            else
            //                bSuccessLoadingOutbox    =
            // theFromOutbox.GenerateLedger(IDFromAccount, SERVER_ID,
            // OTLedger::outbox, true); // bGenerateFile=true
                                        ...
                                        // verifies.
            {
                ...
                OTLog::Error(
                    "ERROR generating ledger in OTServer::NotarizeTransfer.\n");
            }
            else {
                ...
                if (!(pBalanceItem->VerifyBalanceStatement(
                    ...
                    if (theFromAccount.Debit(
                        ...
                        pOutboxTransaction = NULL; // I can't use OTCleanup here
                                                   // because sometimes we DON'T
                                                   // delete it. (above)
                        delete pInboxTransaction;
                        pInboxTransaction = NULL;
                                  ...
                                  bool& bOutSuccess)
{
    ...
    // here.
    const OTIdentifier SERVER_ID(m_strServerID), USER_ID(theNym),
        ACCOUNT_ID(theAccount), SERVER_USER_ID(m_nymServer),
    ...
    if (NULL == pItemVoucher) {
        ...
        pItem = pItemCash;
        if (NULL != pItem) theReplyItemType = OTItem::atWithdrawal;
    }
    else {
        pItem = pItemVoucher;
    ...
    if (NULL == pItem) {
        OTString strTemp(tranIn);
        OTLog::vOutput(
            0, "OTServer::NotarizeWithdrawal: Expected OTItem::withdrawal or "
    ...
    //
             ...
             (pBalanceItem = tranIn.GetItem(OTItem::balanceStatement))) {
        OTString strTemp(tranIn);
        OTLog::vOutput(0, "OTServer::NotarizeWithdrawal: Expected "
                                                                  ...
                                                                  // verifies...
        {
            ...
            if (!bLoadContractFromString) {
                OTLog::vError("OTServer::%s: ERROR loading voucher request "
                              "from string:\n%s\n",
                ...
                OTLog::vError(
                    "OTServer::%s: Failed verifying asset type ID (%s) on the "
            ...
            else // successfully loaded the voucher request from the string...
            {
                ...
                if (bIssueVoucher && (lAmount > 0) &&
                    ...
                    if (false == pVoucherReserveAcct->Credit(
                                     theVoucherRequest.GetAmount())) {
                        OTLog::Error("OTServer::NotarizeWithdrawal: Failed "
                                     "crediting voucher reserve account.\n");

                        if (false ==
                            theAccount.Credit(theVoucherRequest.GetAmount()))
                            OTLog::Error("OTServer::NotarizeWithdrawal "
        ...
        //
            ...
            OTLog::Output(0, "Error: 'From' account ID on the transaction does "
                             ...
                             "item.\n");
                                   ...
                                   // Already verified in OTAccount::LoadInbox()
        {
            OTLog::Error("Error loading or verifying inbox.\n");
        }
        else if (NULL == pOutbox) // || !pOutbox->VerifyAccount(m_nymServer))
                                    // Already verified in
                                    // OTAccount::LoadOutbox()
        {
            OTLog::Error("Error loading or verifying outbox.\n");
        }
        else {
            ...
            else // successfully loaded the purse from the string...
            {
                ...
                while ((pToken = thePurse.Pop(m_nymServer)) != NULL) {
                    ...
                    if (NULL == pMint) {
                        OTLog::vError("OTServer::NotarizeWithdrawal: Unable to "
                    ...
                    else if (NULL == (pMintCashReserveAcct =
                                            pMint->GetCashReserveAccount())) {
                        OTLog::vError(
                            "OTServer::NotarizeWithdrawal: Unable to find cash "
                    ...
                    //
                        ...
                        OTLog::vError(
                            "OTServer::NotarizeWithdrawal: User attempting "
                        ...
                        //
                        else if (!(pMint->SignToken(m_nymServer, *pToken,
                                                    theStringReturnVal,
                                                    0))) // nTokenIndex = 0 //
                            // ******************************************
                        {
                            bSuccess = false;
                            OTLog::vError("%s: Failure in call: "
                                          "pMint->SignToken(m_nymServer, "
...
///
void OTServer::NotarizePayDividend(OTPseudonym& theNym,
                                   ...
                                   OTTransaction& tranOut, bool& bOutSuccess)
{
    ...
    OTItem* pItem = NULL; // This pointer and the following one, are 2 pointers,
                          // as a vestige
    OTItem* pItemPayDividend = NULL; // from the withdrawal code, which has two
                                     // forms: voucher and cash.
    OTItem* pBalanceItem =
        NULL; // The balance agreement item, which must be on any transaction.
    ...
    //
    const OTIdentifier SERVER_ID(m_strServerID), USER_ID(theNym),
        SOURCE_ACCT_ID(theSourceAccount), SERVER_USER_ID(m_nymServer),
        ...
        if (!bLoadContractFromString) {
                     ...
                     SHARES_ASSET_ID) // these can't be the same
            {
                ...
                OTLog::vError("%s: ERROR dividend payout attempted, using "
                     ...
                     pSharesIssuerAccount->VerifyAccount(m_nymServer)) {
            ...
            //
                                         ...
                                         // verifies.
                {
                    OTLog::vError("%s: Error loading or verifying inbox.\n",
                           ...
                           pOutbox) // || !pOutbox->VerifyAccount(m_nymServer))
                                    // OTAccount::Load (above) already verifies.
                {
                                           ...
                                           // nym...
                {
                    ...
                    else // successfully verified the balance agreement.
                    {
                        ...
                        if ((lTotalCostOfDividend > 0) &&
                            ...
                            else // By this point, we have taken the full funds
                            ...
                            { // reserve account. So now, let's iterate all the
                                ...
                                OTAcctFunctor_PayDividend actionPayDividend(
                                    ...
                                    *this, lAmountPerShare, &theAccounts);
                               ...
                               bool& bOutSuccess)
{
    ...
    OTAccount* pMintCashReserveAcct =
        NULL; // the Mint's funds for cash withdrawals.
    ...
    if (NULL == pItem) {
        OTString strTemp(tranIn);
        OTLog::vOutput(0, "OTServer::NotarizeDeposit: Expected OTItem::deposit "
    ...
    //
        ...
        OTString strTemp(tranIn);
        OTLog::vOutput(
            0, "OTServer::NotarizeDeposit: Expected OTItem::balanceStatement, "
    ...
    // cheque is the top half.)
        ...
        if (NULL == pInbox) // || !pInbox->VerifyAccount(m_nymServer)) Verified
                            // in OTAccount::Load
        {
            OTLog::Error("OTServer::NotarizeDeposit: Error loading or "
        ...
        // does not match the "Acct From" ID on this transaction item
            ...
            // just an oversight.)
            OTLog::Output(0, "OTServer::NotarizeDeposit: Error: account ID "
            ...
            {
                //                OTLog::vError("OTServer::NotarizeDeposit:
                // Error: Unable to deposit into the same account cheque was
                // drawn on:\n%s\n",
                //                              strCheque.Get());
            ...
            else  // BELOW: Cheque deposit
            {
                ...
                OTAccount* pRemitterAcct =
                    NULL; // Only used in the case of vouchers.
                OTCleanup<OTAccount> theRemitterAcctGuardian; // This is set
                                                              // below, right
                                                              // after
                // OTAccount::LoadExistingAccount().
                OTAccount* pSourceAcct = NULL; // We'll load this up and change
                ...
                { // (If it's not a voucher, that means we should DEFINITELY be
                    ...
                    if (bSuccessLoadSenderInbox)
                        bSuccessLoadSenderInbox =
                            pSenderInbox->VerifyAccount(m_nymServer);
                    else
                        OTLog::vOutput(0, "OTServer::%s: Failed loading inbox "
                ...
                if (bHasRemitter && !bSenderIsServer) {
                    OTLog::vOutput(0, "OTServer::%s: Failure: Voucher has a "
                                      "'sender' who is not the server: %s\n",
                                   __FUNCTION__, strSenderUserID.Get());
                }
                //              else if (bRemitterIsServer) // Disallowed for
                // now. UPDATE: Necesssary for paying dividends.
                //              {
                //                  OTLog::vOutput(0, "OTServer::%s: Failure:
                // Voucher has the server set as the 'remitter'
                // (disallowed.)\n",
                //                                 __FUNCTION__);
                //              }
                ...
                // person.)
                         ...
                         //                      (false ==
                         // theSenderNym.LoadCredentials()) &&  // New style.
                         (false ==
                    ...
                    OTLog::vOutput(
                        0, "OTServer::%s: Error loading public key for %s "
                           "signer during "
                           "deposit: %s\nRecipient User ID: %s\n",
                ...
                // digest of his public key.
                    ...
                    OTLog::vOutput(
                        0, "OTServer::%s: Failure verifying %s: "
                ...
                // person.)
                         ...
                         (false ==
                          theSenderNym.LoadSignedNymfile(m_nymServer))) {
                    OTLog::vOutput(
                        0, "OTServer::%s: Error loading nymfile for %s signer "
                ...
                // the same person.)
                         ...
                         //                  (false ==
                         // theRemitterNym.LoadCredentials())      &&  // New
                         // style.
                         (false ==
                    ...
                    OTLog::vOutput(
                        0, "OTServer::%s: Error loading public key for "
                           "remitter during "
                           "voucher deposit: %s\nRecipient User ID: %s\n",
                ...
                // digest of his public key.
                    ...
                    OTLog::vOutput(
                        0, "OTServer::%s: Failure verifying voucher: "
                ...
                // person.)
                         ...
                         (false ==
                          theRemitterNym.LoadSignedNymfile(m_nymServer))) {
                    OTLog::vOutput(0, "OTServer::%s: Error loading nymfile for "
                ...
                {
                    OTLog::vOutput(
                        0, "OTServer::%s: Failure verifying %s signature for "
                ...
                // account...
                    ...
                    OTLog::vOutput(0, "OTServer::%s: ERROR verifying signer's "
                    ...
                    if (bSourceAcctIsRemitter) {
                        ...
                        if (NULL != pRemitterInbox) // This part is here for
                    ...
                    //
                             ...
                             !pRemitterAcct->VerifySignature(m_nymServer)) {
                    ...
                    // remitter account...
                        ...
                        OTLog::vOutput(
                            0, "OTServer::%s: ERROR verifying remitter's "
                            ...
                            __FUNCTION__, strRemitterAcctID.Get());
                             ...
                             (!pRemitterInbox->LoadInbox() ||
                              !pRemitterInbox->VerifyAccount(
                                   m_nymServer))) // Then load and verify it.
                    {
                        OTLog::vError("%s: ERROR loading or verifying inbox "
                    ...
                    // Asset Type?
                        ...
                        OTLog::vOutput(
                            0, "OTServer::%s: ERROR - user attempted to "
                    ...
                    */
                        ...
                        OTLog::vOutput(0, "OTServer::%s: ERROR verifying "
                    ...
                    //
                        ...
                        if (false ==
                            ...
                            OTLog::vError("OTServer::%s: Failed debiting "
                                          "source account.\n",
                                          __FUNCTION__);
                        }
                        else if (false ==
                                   theAccount.Credit(theCheque.GetAmount())) {
                            OTLog::vError("OTServer::%s: Failed crediting "
                                          "destination account.\n",
                                          __FUNCTION__);
                                       ...
                                       ) 
                        {
                            ...
                            if (false ==
                                pSourceAcct->Credit(theCheque.GetAmount()))
                                OTLog::vError("OTServer::%s: Failed "
                            ...
                            if (bRemitterCancelling) {
                                tranOut.SetAsCancelled();
                                OTLog::vOutput(1, "OTServer::%s: SUCCESS "
    ...
    // depositCheque (the item)
        ...
        if (ACCOUNT_ID != pItem->GetPurportedAccountID()) {
            OTLog::vOutput(0, "OTServer::NotarizeDeposit: Error: 'From' "
                              "account ID on the transaction does not match "
            ...
            if (!bLoadContractFromString) {
                OTLog::vError("OTServer::NotarizeDeposit: ERROR loading purse "
                              "from string:\n%s\n",
                              strPurse.Get());
            }
            else if (!(pBalanceItem->VerifyBalanceStatement(
                            ...
                            theAccount, tranIn))) {
                OTLog::vOutput(0, "OTServer::NotarizeDeposit: ERROR verifying "
            ...
            else // the purse loaded successfully from the string
            {
                ...
                while ((pToken = thePurse.Pop(m_nymServer)) != NULL) {
                    ...
                    if (NULL == pMint) {
                        OTLog::Error("OTServer::NotarizeDeposit: Unable to get "
                                     "or load Mint.\n");
                        break;
                    }
                    else if ((pMintCashReserveAcct =
                                    pMint->GetCashReserveAccount()) != NULL) {
                        ...
                        {
                            bSuccess = false;
                            OTLog::vOutput(0, "OTServer::NotarizeDeposit: "
                        ...
                        // that it hasn't already been spent...
                            ...
                            OTLog::Output(3, "OTServer::NotarizeDeposit: "
                            ...
                            if (false == pMintCashReserveAcct->Debit(
                                             pToken->GetDenomination())) {
                                OTLog::Error("OTServer::NotarizeDeposit: Error "
                            ...
                            // CREDIT the amount to the account...
                                ...
                                if (false == pMintCashReserveAcct->Credit(
                                                 pToken->GetDenomination()))
                                    OTLog::Error("OTServer::NotarizeDeposit: "
                            ...
                            {
                                OTLog::vOutput(2, "OTServer::NotarizeDeposit: "
                    ...
                    //                    delete pToken; // Unnecessary now,
                    // handled by OTCleanup.
                    //                    pToken = NULL;
                ...
                if (bSuccess) {
                    ...
                    OTLog::Output(1, "OTServer::NotarizeDeposit: .....SUCCESS "
...
///
///
void OTServer::NotarizePaymentPlan(OTPseudonym& theNym,
                                   ...
                                   OTTransaction& tranOut, bool& bOutSuccess)
{
    ...
    const OTIdentifier SERVER_ID(m_strServerID), DEPOSITOR_USER_ID(theNym),
        SERVER_USER_ID(m_nymServer), DEPOSITOR_ACCT_ID(theDepositorAccount),
        USER_ID(theNym);
    ...
    // So we treat it that way... I either get it successfully or not.
                              ...
                              theNym, tranIn)) // bIsRealTransaction=true
        {
            ...
            //            else if (!pPlan->VerifySignature(theNym))  // This is
            // now done below, in VerifyAgreement()!
            //            {
            //                OTLog::vOutput(0, "ERROR verifying sender
            ...
            //            }
                ...
                else // The plan is good (so far.)
                {
                    ...
                    OTPseudonym theRecipientNym; // We'll probably use this, but
                                                 ...
                                                 // point here.
                    OTPseudonym* pRecipientNym = NULL; // Here's the pointer.
                                                       // (Logic explained
                                                       // directly below.)
                        ...
                        if (!bCancelling &&
                            ...
                            // had a chance to confirm/activate it.
                                        ...
                                        OTTimeGetCurrentTime())) // bSaveReceipt=true
                                {
                                    ...
                                    theIDSet2.insert(
                                        pPlan->GetRecipientClosingNum());
                                    //                                  pRecipientNym->SaveSignedNymfile(m_nymServer);
                                    // // saved below
                                ...
                                } // if (m_Cron.AddCronItem(*pPlan, &theNym,
                                  // true, OTTimeGetCurrentTime())) //
                                  // bSaveReceipt=true
                                else {
                                    if (bCancelling) {
                                        tranOut.SetAsCancelled();
...
// Done.
//
void OTServer::NotarizeSmartContract(OTPseudonym& theNym,
                                ...
                                theNym, tranIn)) // bIsRealTransaction=true
        {
            ...
            if ((false == pContract->LoadContractFromString(strContract))) {
                ...
                // the activator!
                         ...
                         (NULL !=
                          pContract->FindPartyBasedOnNymAsAgent(m_nymServer))) {
                    OTLog::vOutput(
                        0,
                ...
                */
                ...
                //
                              ...
                              theNym, theActivatingAccount, m_nymServer,
                    ...
                    */
                    ...
                    if (false ==
                        pContract->SendNoticeToAllParties(
                            false, // bSuccessMsg=false (OTItem::rejection)
                            m_nymServer, SERVER_ID, lNewTransactionNumber,
                            //                                                               pContract->GetTransactionNum(),
                            // // Each party has its own opening number. Handled
                            // internally.
                            strContract, NULL, NULL, &theNym)) {
                        ...
                        //                    OTLog::vOutput(0, "%s: Failed
                        // notifying all parties about failed activation of
                        // smart contract: %ld.\n",
                        //                                   __FUNCTION__,
                        // pContract->GetTransactionNum());
                    }
...
//
void OTServer::NotarizeCancelCronItem(OTPseudonym& theNym,
        ...
        // **********************************************************************
            ...
            else // LET'S SEE IF WE CAN REMOVE IT THEN...
            {
                OTCronItem* pCronItem =
                    m_Cron.GetItemByValidOpeningNum(lReferenceToNum);
                ...
                if ((NULL != pCronItem) &&
                    (pCronItem->CanRemoveItemFromCron(theNym))) // see if theNym
                                                                ...
                                                                // processing.
                {
                    bSuccess = m_Cron.RemoveCronItem(
                ...
                if (bSuccess) {
                    ...
                    OTLog::Output(0, "Unable to remove Cron Item from Cron "
                                     "object "
                                     "OTServer::NotarizeCancelCronItem\n");
                }
            }
...
/// void OTServer::NotarizeExchangeBasket(OTPseudonym & theNym, OTAccount &
/// theAccount, OTTransaction & tranIn, OTTransaction & tranOut)
///
/// a user is exchanging in or out of a basket.  (Ex. He's trading 2 gold and 3
/// silver for 10 baskets, or vice-versa.)
///
void OTServer::NotarizeExchangeBasket(OTPseudonym& theNym,
                                      ...
                                      OTTransaction& tranOut, bool& bOutSuccess)
{
    ...
    if (false == NYM_IS_ALLOWED(strUserID.Get(), __transact_exchange_basket)) {
        ...
        OTLog::Output(0, "OTServer::NotarizeExchangeBasket: No exchangeBasket "
                         "item found on this transaction.\n");
    }
    else if (NULL == pBalanceItem) {
        OTLog::Output(0, "OTServer::NotarizeExchangeBasket: No Balance "
                                  ...
                                  // OTAccount::Load (above) already verifies.
    {
        ...
        if (false == pBalanceItem->VerifyBalanceStatement(
                         ...
                         *pOutbox, theAccount, tranIn)) {
            OTLog::vOutput(0, "OTServer::NotarizeExchangeBasket: ERROR "
        ...
        else // BALANCE AGREEMENT WAS SUCCESSFUL.......
        {
            ...
            if (!bLookup) {
                ...
                // If the pointer is NULL, that works too. Otherwise it cleans
                ...
                if (NULL == pBasketAcct) {
                    OTLog::Error("ERROR loading the basket account in "
                                 "OTServer::NotarizeExchangeBasket\n");
                }
                // Does it verify?
                // I call VerifySignature here since VerifyContractID was
                // already called in LoadExistingAccount().
                        ...
                        if (!bFoundSameAcctTwice) // Let's do it!
                        {
                            ...
                            for (int32_t i = 0; i < theBasket.Count(); i++) {
                                ...
                                else // if equal
                                {
                                    ...
                                    //
                                    OTLedger* pSubInbox =
                                        pUserAcct->LoadInbox(m_nymServer);

                                    if (NULL == pSubInbox) // ||
                                        // !pSubInbox->VerifyAccount(m_nymServer))
                                        // OTAccount::Load (above)
                                        // already verifies.
                                    {
                                    ...
                                    else 
                                    {
                                        ...
                                        if (bSuccess) { // need to be able to
                                            ...
                                            pInboxTransaction->SetClosingNum(
                                                pRequestItem
                                                    ->lClosingTransactionNo); // Here is the number the user wishes to sign-off by accepting this receipt.
...
// Use the CLOSING NUMBERS for each member, and for the basket, to put RECEIPTS
// in their INBOXES!
//
/// a user is exchanging in or out of a basket.  (Ex. He's trading 2 gold and 3
/// silver for 10 baskets, or vice-versa.)
/*
  void OTServer::UserCmdExchangeBasket(OTPseudonym & theNym, OTMessage & MsgIn,
  OTMessage & msgOut)
  {
    // (1) set up member variables
  ...
  BASKET_ACCOUNT_ID);
    ...
    if (bLookup && theRequestBasket.LoadContractFromString(strBasket) &&
  theRequestBasket.VerifySignature(theNym))
    {
  // Load the basket account and make sure it exists.
  pBasketAcct = OTAccount::LoadExistingAccount(BASKET_ACCOUNT_ID, SERVER_ID);

  // If the pointer is NULL, that works too. Otherwise it cleans up the object
  ...
  {
  OTLog::Error("ERROR loading the basket account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  }
  // Does it verify?
  ...
  {
  OTLog::Error("ERROR verifying signature on the basket account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  }
  else
  ...
  {
  BasketItem * pBasketItem    = theBasket.At(i);
  BasketItem * pRequestItem    = theRequestBasket.At(i); // we already know
  these are the same length

  // if not equal
  if (!(pBasketItem->SUB_CONTRACT_ID == pRequestItem->SUB_CONTRACT_ID))
  {
  OTLog::Error("Error: expected asset type IDs to match in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  OTAccount * pUserAcct    =
  OTAccount::LoadExistingAccount(pRequestItem->SUB_ACCOUNT_ID, SERVER_ID);

  if (NULL == pUserAcct)
  {
  OTLog::Error("ERROR loading a user's asset account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  OTAccount * pServerAcct    =
  OTAccount::LoadExistingAccount(pBasketItem->SUB_ACCOUNT_ID, SERVER_ID);

  if (NULL == pServerAcct)
  {
  OTLog::Error("ERROR loading a basket sub-account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  {
  OTLog::Error("ERROR verifying asset type on a user's account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  {
  OTLog::Error("ERROR verifying signature on a user's asset account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  {
  OTLog::Error("ERROR verifying signature on a basket sub-account in
  OTServer::UserCmdExchangeBasket\n");
  msgOut.m_bSuccess = false;
  break;
  }
  ...
  {
  // the amount being transferred between these two accounts is the minimum
  transfer amount
  // for the sub-account on the basket, multiplied by
  lTransferAmount = (pBasketItem->lMinimumTransferAmount *
  theRequestBasket.GetTransferMultiple());
  ...
  {
  msgOut.m_bSuccess = false;
  OTLog::Output(0, "Unable to Debit user account, or Credit server account, in
  OTServer::UserCmdExchangeBasket\n");
  break;
  }
  }
  ...
  {
  msgOut.m_bSuccess = false;
  OTLog::Output(0, "Unable to Credit user account, or Debit server account, in
  OTServer::UserCmdExchangeBasket\n");
  break;
  }
  }
  ...
  {
  OTLog::Error("Error finding asset contract for basket, or loading the basket
  from it, or verifying\n"
  "the signature on that basket, or the request basket didn't match actual
  basket.\n");
  }
  } // pBasket exists and signature verifies
  ...
  *****************************************************************************
        ...
        // Load up the two accounts and perform the exchange...
    // (Above we did the sub-accounts for server and user. Now we do the main
  accounts for server and user.)
    //
        OTAccount * pUserMainAcct    = NULL;
        OTCleanup<OTAccount> theUserAcctGuardian;

        // At this point, if we have successfully debited / credited the
  sub-accounts.
        // then we need to debit and credit the user's main basket account and
  the server's basket issuer account.
        if ((true == msgOut.m_bSuccess) && pBasketAcct &&
    (pUserMainAcct =
  OTAccount::LoadExistingAccount(theRequestBasket.GetRequestAccountID(),
  SERVER_ID)) &&
    theUserAcctGuardian.SetCleanupTargetPointer(pUserMainAcct) && // Make sure
  pUserMainAcct gets cleaned up
    pUserMainAcct->VerifySignature(m_nymServer) &&
    (pUserMainAcct->GetAssetTypeID() == BASKET_CONTRACT_ID)) // Just make sure
  the two main basket accts have same currency type
        {
    lTransferAmount =  (theRequestBasket.GetMinimumTransfer() *
  theRequestBasket.GetTransferMultiple());
    ...
    if (msgOut.m_bBool)
    {
    if (pUserMainAcct->Credit(lTransferAmount) &&
  pBasketAcct->Debit(lTransferAmount))
    {
    msgOut.m_bSuccess = true;
    }
    else
    {
    msgOut.m_bSuccess = false;
    OTLog::Output(0, "Unable to Credit user basket account, or Debit basket
  issuer account, "
    ...
    {
    if (pUserMainAcct->Debit(lTransferAmount) &&
  pBasketAcct->Credit(lTransferAmount))
    {
    msgOut.m_bSuccess = true;
    }
    else
    {
    msgOut.m_bSuccess = false;
    OTLog::Output(0, "Unable to Debit user basket account, or Credit basket
  issuer account, "
    "in OTServer::UserCmdExchangeBasket\n");
    }
    }
    ...
    OTLog::Error("Error loading or verifying user's main basket account in
  OTServer::UserCmdExchangeBasket\n");
    msgOut.m_bSuccess = false;
        }

        // At this point, we have hopefully credited/debited ALL the relevant
  accounts.
        // So now, let's Save them ALL to disk.. (and clean them up.)
        OTAccount * pAccount = NULL;

        // empty the list of USER accounts (and save to disk, if everything was
  successful.)
        while (!listUserAccounts.empty())
        {
    pAccount = listUserAccounts.front();
    ...
    delete pAccount; pAccount=NULL;
        ...
        // empty the list of SERVER accounts (and save to disk, if everything
  was successful.)
        while (!listServerAccounts.empty())
        {
    pAccount = listServerAccounts.front();
    ...
    pUserMainAcct->SaveAccount();
        ...
        // NO need to cleanup pBasketAcct or pUserMainAcct here, since OTCleanup
  handles it now.
    }
...
void OTServer::NotarizeMarketOffer(OTPseudonym& theNym,
                                   OTAccount& theAssetAccount,
                                   OTTransaction& tranIn,
                                   OTTransaction& tranOut, bool& bOutSuccess)
{
    ...
    if (false == NYM_IS_ALLOWED(strUserID.Get(), __transact_market_offer)) {
        ...
        OTString strTemp(tranIn);
        OTLog::vOutput(0, "OTServer::NotarizeMarketOffer: Expected transaction "
        ...
        OTString strTemp(tranIn);
        OTLog::vOutput(0, "OTServer::NotarizeMarketOffer: Expected "
                         ...
                         theNym, tranIn))) // bIsRealTransaction = true;
        {
            ...
            // ok so the IDs match. Does the currency account exist?
                ...
                OTLog::Output(0, "ERROR verifying existence of the currency "
                                 "account in OTServer::NotarizeMarketOffer\n");
            }
            ...
            // also be a closing number for closing it.
                ...
                OTLog::vOutput(0, "OTServer::NotarizeMarketOffer: ERROR wrong "
            ...
            // offer.
                ...
                //
                OTLog::Output(0, "OTServer::NotarizeMarketOffer: FAILED adding "
                                 "offer to market: "
...
void OTServer::NotarizeTransaction(OTPseudonym& theNym, OTTransaction& tranIn,
                                   OTTransaction& tranOut, bool& bOutSuccess)
{
...
void OTServer::UserCmdNotarizeTransactions(OTPseudonym& theNym,
                                           OTMessage& MsgIn, OTMessage& msgOut)
{
    ...
    const OTIdentifier USER_ID(MsgIn.m_strNymID), ACCOUNT_ID(MsgIn.m_strAcctID),
        SERVER_ID(m_strServerID), SERVER_USER_ID(m_nymServer);
    ...
    if (msgOut.m_bSuccess) {
        ...
        FOR_EACH(mapOfTransactions, theLedger.GetTransactionMap())
        {
            ...
            pTranResponse = NULL; // at this point, the ledger now "owns" the
        ...
        OTLog::Error("ERROR loading ledger from message in "
                     "OTServer::UserCmdNotarizeTransactions\n");
    }
                                       ...
                                       OTPseudonym* pActualNym /*=NULL*/)
{
    ...
    if (true == bSuccessLoadingNymbox)
        bSuccessLoadingNymbox = (theNymbox.VerifyContractID() &&
                                 theNymbox.VerifySignature(m_nymServer));
    //        bSuccessLoadingNymbox    = theNymbox.VerifyAccount(m_nymServer);
        ...
        if (!bGotNextTransNum) {
            ...
            if ((NULL != pActualNym) && pActualNym->CompareID(USER_ID)) {
                ...
                OTLog::Error("OTServer::DropReplyNoticeToNymbox: ERROR: "
                             "pActualNym was not NULL, but it didn't match "
                             "USER_ID.\n");
        }
    }
}
...
void OTServer::UserCmdGetAccount(OTPseudonym&, OTMessage& MsgIn,
                                 OTMessage& msgOut)
{
    // (1) set up member variables
...
void OTServer::UserCmdGetAccountFiles(OTPseudonym&, OTMessage& MsgIn,
                                      OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (bSuccessLoadingAccount) {
        ...
        //
        {
            ...
            if (!bSuccessLoadingInbox)
                ...
                //
                bSuccessLoadingInbox = (theInbox.VerifyContractID() &&
                                        theInbox.VerifySignature(m_nymServer));
            ...
            if (!bSuccessLoadingOutbox)
                ...
                bSuccessLoadingOutbox =
                    (theOutbox.VerifyContractID() &&
                     theOutbox.VerifySignature(m_nymServer));
                ...
                {
                    //                  bSuccessLoadingOutbox =
                    // theOutbox.VerifyAccount(m_nymServer);    // Then Verify,
                    // which forces a LoadBoxReceipts... (

                    theOutbox.ReleaseSignatures(); // UPDATE: We do NOT force
                                                   // the loading here, since
                                                   // they aren't needed.
                    theOutbox.SignContract(m_nymServer); // Waste of resources.
                                                         // Instead, we
                                                         // recognize that it
                                                         // was old data, and so
...
// Deprecated (replaced by UserCmdGetAccountFiles)
void OTServer::UserCmdGetInbox(OTPseudonym&, OTMessage& MsgIn,
                               OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (!msgOut.m_bSuccess)
        ...
        {
            //            msgOut.m_bSuccess =
            // theLedger.VerifyAccount(m_nymServer);    // Then Verify, which
            // forces a LoadBoxReceipts... (

            theLedger.ReleaseSignatures(); // UPDATE: We do NOT force the
                                           // loading here, since they aren't
                                           // needed.
            theLedger.SignContract(m_nymServer); // Waste of resources. Instead,
...
// Deprecated (replaced by UserCmdGetAccountFiles)
void OTServer::UserCmdGetOutbox(OTPseudonym&, OTMessage& MsgIn,
                                OTMessage& msgOut)
{
    // (1) set up member variables
...
void OTServer::UserCmdQueryAssetTypes(OTPseudonym&, OTMessage& MsgIn,
                                      OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (MsgIn.m_ascPayload.Exists()) // (which it should)
    {
        ...
        OTCleanup<OTDB::Storable> theAngel(
            pStorable); // It will definitely be cleaned up.
        OTDB::StringMap* pMap = (NULL == pStorable)
                                    ? NULL
                                    : dynamic_cast<OTDB::StringMap*>(pStorable);
...
void OTServer::UserCmdGetContract(OTPseudonym&, OTMessage& MsgIn,
                                  OTMessage& msgOut)
{
    // (1) set up member variables
...
// Done.
//
void OTServer::UserCmdTriggerClause(OTPseudonym& theNym, OTMessage& MsgIn,
                                    OTMessage& msgOut)
{
    OTString strInReferenceTo(
        MsgIn); // Grab the incoming message in plaintext form
    ...
    if ((bGotNymboxHashServerSide && bGotNymboxHashClientSide &&
        ...
        OTCronItem* pCronItem =
            m_Cron.GetItemByValidOpeningNum(MsgIn.m_lTransactionNum);
    ...
    // (2) Sign the Message
    msgOut.SignContract(
        (const OTPseudonym&)m_nymServer); // todo change this cast.
...
// fails.)
//
void OTServer::UserCmdDeleteUser(OTPseudonym& theNym, OTMessage& MsgIn,
                                 OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    const bool bSuccessLoadNymbox =
        (theLedger.LoadNymbox() && theLedger.VerifyAccount(m_nymServer));
...
// for an Asset Acct.
//
void OTServer::UserCmdGetBoxReceipt(OTPseudonym&, OTMessage& MsgIn,
                                    OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    case 0: // Nymbox
        ...
        {
            OTLog::vError(
                "OTServer::UserCmdGetBoxReceipt: User requested Nymbox, but "
    ...
    case 1: // Inbox
        if (USER_ID == ACCOUNT_ID) {
            OTLog::vError("OTServer::UserCmdGetBoxReceipt: User requested "
    ...
    if (bSuccessLoading && !bErrorCondition &&
        //        pLedger->VerifyAccount(m_nymServer)    &&    // This call
        ...
        pLedger->VerifySignature(
            m_nymServer) // just load the one we actually need.
        ) {
        OTTransaction* pTransaction =
            pLedger->GetTransaction(MsgIn.m_lTransactionNum);
        if (NULL == pTransaction) {
            OTLog::vError("OTServer::UserCmdGetBoxReceipt: User requested a "
                          "transaction number "
            ...
            if ((NULL != pTransaction) && !pTransaction->IsAbbreviated() &&
                ...
                OTLog::vOutput(
                    3, "OTServer::UserCmdGetBoxReceipt: Success: User is "
                ...
                OTLog::vError(
                    "OTServer::UserCmdGetBoxReceipt: User requested a "
        ...
        OTLog::vError(
            "OTServer::UserCmdGetBoxReceipt: Failed loading or verifying %s. "
...
// well!
//
void OTServer::UserCmdDeleteAssetAcct(OTPseudonym& theNym, OTMessage& MsgIn,
                                      OTMessage& msgOut)
{
    ...
    if (NULL == pAccount || !pAccount->VerifyAccount(m_nymServer)) {
        ...
        if (NULL == pInbox) // || !pInbox->VerifyAccount(m_nymServer)) // NOTE:
        ...
        else // SUCCESS!
        {
            ...
            theNym.SaveSignedNymfile(m_nymServer);
            OTAssetContract* pContract =
                GetAssetContract(pAccount->GetAssetTypeID());
...
void OTServer::UserCmdGetNymbox(OTPseudonym& theNym, OTMessage& MsgIn,
                                OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    if (!msgOut.m_bSuccess)
        OTLog::Error("OTServer::UserCmdGetNymbox: Failed trying to load Nymbox "
                     "from storage.\n");
    else {
        // We do NOT call VerifyAccount in this function (because we don't need
        // to) and thus we do NOT
        ...
        if (!msgOut.m_bSuccess)
            OTLog::Error("OTServer::UserCmdGetNymbox: Verification failed on "
...
void OTServer::UserCmdProcessNymbox(OTPseudonym& theNym, OTMessage& MsgIn,
                                    OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    const OTIdentifier USER_ID(msgOut.m_strNymID), SERVER_ID(m_strServerID),
        SERVER_USER_ID(m_nymServer);
...
//
//
void OTServer::NotarizeProcessNymbox(OTPseudonym& theNym, OTTransaction& tranIn,
                                     OTTransaction& tranOut, bool& bOutSuccess)
{
    ...
    if (true == bSuccessLoadingNymbox)
        bSuccessLoadingNymbox =
            theNymbox.VerifyAccount(m_nymServer); // make sure it's all good.
    pResponseBalanceItem = OTItem::CreateItemFromTransaction(
        tranOut, OTItem::atTransactionStatement);
    pResponseBalanceItem->SetStatus(OTItem::rejection); // the default.
    tranOut.AddItem(*pResponseBalanceItem); // the Transaction's destructor will
    ...
    if (false == bSuccessLoadingNymbox) {
        OTLog::vOutput(
            0,
            "OTServer::%s: Failed loading or verifying Nymbox for user:\n%s\n",
            __FUNCTION__, strNymID.Get());
    }
    else if (NULL == pBalanceItem) {
        const OTString strTransaction(tranIn);
        OTLog::vOutput(0, "OTServer::%s: No Transaction Agreement item found "
            ...
            if (pItem->GetType() == OTItem::acceptTransaction) {
                                            ...
                                            // actually there.
                {
                    ...
                    if (false == listNumbersNymbox.Verify(listNumbersUserItem))
                        OTLog::Error(
                            "OTServer::NotarizeProcessNymbox: Failed "
                    ...
                    {    // we actually add an entire list of numbers retrieved
                        ...
                        FOR_EACH(std::set<int64_t>, theNumbers)
                        {
                            ...
                            //
                            if (false ==
                                theNym.VerifyIssuedNum(m_strServerID,
                                                       lTransactionNumber)) {
                                theNym.AddIssuedNum(m_strServerID,
                                                    lTransactionNumber);
                                ...
                                OTLog::vError("OTServer::NotarizeProcessNymbox:"
        ...
        else // TRANSACTION AGREEMENT WAS SUCCESSFUL.......
        {
            ...
            FOR_EACH(listOfItems, tranIn.GetItemList())
            {
                ...
                if ((OTItem::request == pItem->GetStatus()) &&
                    ...
                    if ((NULL != (pServerTransaction = theNymbox.GetTransaction(
                        ...
                        if ((OTItem::acceptMessage == pItem->GetType()) &&
                            ...
                            pServerTransaction->DeleteBoxReceipt(
                                theNymbox); // faster.
                            //                            theNymbox.
                            // DeleteBoxReceipt(pServerTransaction->GetTransactionNum());
                        ...
                        // for cash withdrawals as well...)
                            ...
                            if (!bGotNextTransNum) {
                                ...
                                OTLog::Error("Error getting next transaction "
                                             "number in "
                                             "OTServer::NotarizeProcessNymbox "
                                ...
                                if (NULL != pSuccessNotice) // The above has an
                                                            // OT_ASSERT within,
                                                            // but I just like
                                                            // to check my
                                                            // pointers.
    ...
    if (bNymboxHashRegenerated) {
        ...
        theNym.SaveSignedNymfile(m_nymServer); // todo: make objects like nym
    ...
    if ((NULL != pResponseBalanceItem) &&
        ...
        if (tranOut.GetSuccess()) {
            ...
            theNym.HarvestIssuedNumbers(SERVER_ID, m_nymServer, theTempNym,
                                        true); // bSave=true
...
void OTServer::UserCmdProcessInbox(OTPseudonym& theNym, OTMessage& MsgIn,
                                   OTMessage& msgOut)
{
    // (1) set up member variables
    ...
    const OTIdentifier USER_ID(msgOut.m_strNymID),
        ACCOUNT_ID(MsgIn.m_strAcctID), SERVER_ID(m_strServerID),
        SERVER_USER_ID(m_nymServer);
    ...
    OTLedger* pResponseLedger =
        OTLedger::GenerateLedger(SERVER_USER_ID, ACCOUNT_ID, SERVER_ID,
                                 OTLedger::message, false); // bCreateFile=false
    OT_ASSERT_MSG(
        NULL != pResponseLedger,
        "OTServer::UserCmdProcessInbox: ASSERT: NULL != pResponseLedger");
    OTCleanup<OTLedger> theRespLedgerGuardian(*pResponseLedger);
    ...
    if (msgOut.m_bSuccess) {
        ...
        if (!theAccount.LoadContract()) {
            ...
            OTLog::vOutput(
                0,
                "OTServer::UserCmdProcessInbox: Failed loading account: %s\n",
                strAcctID.Get());
        }
        // Make sure the account isn't marked for deletion.
        else if (theAccount.IsMarkedForDeletion()) {
            OTLog::vOutput(0, "OTServer::UserCmdProcessInbox: Failed attempt "
        ...
        // and beyond that method.
            ...
            if (NULL == pTransaction) // I'm assuming there's only one in the
                                      // ledger (for now anyways..)
            {
                ...
                OT_ASSERT_MSG(
                    NULL != pTranResponse,
                    "OTServer::UserCmdProcessInbox: NULL != pTranResponse");
                ...
                //
                    ...
                    OTLog::Output(0, "Error verifying transaction items "
                                     "OTServer::UserCmdProcessInbox\n");
                }
...
send_message:
    ...
    if (NULL == pTranResponse) {
        pTranResponse = OTTransaction::GenerateTransaction(
            *pResponseLedger, OTTransaction::error_state, 0);
        OT_ASSERT_MSG(NULL != pTranResponse,
                      "OTServer::UserCmdProcessInbox 2: NULL != pTranResponse");
    ...
    // sign the outoing transaction
    OT_ASSERT_MSG(NULL != pTranResponse,
                  "OTServer::UserCmdProcessInbox 3: NULL != pTranResponse");
...
/// accordingly.
/// (And each of those transactions must be accepted or rejected in whole.)
void OTServer::NotarizeProcessInbox(OTPseudonym& theNym, OTAccount& theAccount,
                                    OTTransaction& tranIn,
                                    OTTransaction& tranOut, bool& bOutSuccess)
{
                               ...
                               // already verified in OTAccount::LoadInbox()
    {
        OTLog::vError("%s: Error loading or verifying inbox.\n", __FUNCTION__);
                                ...
                                // already verified in OTAccount::LoadOutbox()
    {
        OTLog::vError("%s: Error loading or verifying outbox.\n", __FUNCTION__);
    }
    else {
        ...
        // ENTERING LOOP ------ (Loaded "
        //                      "Inbox for ACCOUNT ID): %s.\n ==========> Inbox
        // Receipt Count: %d \n",
        //                      strAccountIDBLAH.Get(),
        ...
        //      std::set<int64_t>  setOfRefNumsProcessed; // To make sure each
        ...
        FOR_EACH_IT(listOfItems, tranIn.GetItemList(), it_bigloop)
        {
            ...
            case OTItem::balanceStatement:
                //                  OTLog::Output(1,
                // "OTServer::NotarizeProcessInbox: (This item is a balance
                // statement... SKIPPING IT.)\n\n"); // temp remove
            ...
            case OTItem::disputeBasketReceipt:
                //
                //                  OTLog::vOutput(1,
                // "OTServer::NotarizeProcessInbox: Checking server-side inbox
                // for expected CRON receipt: %ld... \n",
                //                                 pItem->GetReferenceToNum());
                // // temp remove
                // NOTE: Turns out, it's normal to have multiple receipts in
                // reference to the same thing.
                // For example, I might have two transfer receipts that are both
                // in reference to the same notarizeInbox.
                //                    {
                //
                //                        std::set<int64_t>::iterator it_ref =
                // setOfRefNumsProcessed.find(pItem->GetReferenceToNum()); // It
                // had BETTER not be on this list already!
                //
                //                        if (setOfRefNumsProcessed.end() !=
                // it_ref) // It's already there!! (Bad.)
                //                        {
                //                            OTLog::vError("%s: 1. Bad input
                // from client: This request already contains an item in
                // reference to receipt %ld, yet here's "
                //                                          "ANOTHER item in
                // reference to the same number! (Failure.)\n", __FUNCTION__,
                // pItem->GetReferenceToNum());
                //                            bSuccessFindingAllTransactions =
                // false;
                //                            break;
                //                        }
                //                        else
            ...
            case OTItem::disputeItemReceipt:
                ...
                //                  OTLog::vOutput(1,
                // "OTServer::NotarizeProcessInbox: Checking server-side inbox
                // for receipt pending (IN REF TO): %ld... \n",
                //                                 pItem->GetReferenceToNum());
                // // temp remove
                // NOTE: Turns out, it's normal to have multiple receipts in
                // reference to the same thing.
            ...
            case OTItem::acceptFinalReceipt:
                ...
                {
                    //                  int32_t nRefCount = 0; // Using std::set
                    // here instead of a simple int32_t. (To prevent
                    // duplicates.)
            ...
            case OTItem::acceptBasketReceipt:
                ...
                //
                if (theNym.VerifyIssuedNum(m_strServerID,
                                           pServerTransaction->GetClosingNum()))
                    theTempNym.AddIssuedNum(
                        m_strServerID, pServerTransaction->GetClosingNum());
                else {
                    bSuccessFindingAllTransactions = false;
            ...
            default:
                OTLog::Error("Wrong item type in "
                             "OTServer::NotarizeProcessInbox. (2nd notice.)\n");
                bSuccessFindingAllTransactions = false;
                break;
            } // switch --------------------------------------------
        ...
        if (false == bSuccessFindingAllTransactions) {
            ...
            if (false == bVerifiedBalanceStatement) {
                OTLog::vOutput(0, "OTServer::NotarizeProcessInbox: ERROR "
                ...
                FOR_EACH_IT(listOfItems, tranIn.GetItemList(), it_bigloop_two)
                {
                    ...
                    if ((OTItem::request == pItem->GetStatus()) &&
                        ((OTItem::acceptCronReceipt ==
                         ...
                         // == pItem->GetType()) ||    // Disputing said notice.
                         // With Cron receipts, original is stored as an
                         // OTCronItem...
                         (OTItem::acceptItemReceipt ==
                          pItem->GetType()) || // Accepted item receipt (cheque,
                                               // transfer)
                         //                       (OTItem::disputeItemReceipt
                         // == pItem->GetType()) ||    // Disputing said notice.
                         (OTItem::acceptPending ==
                          pItem->GetType()) || // Accepting notice of pending
                                               // transfer
                         //                       (OTItem::rejectPending
                         // == pItem->GetType()) ||    // With pending, the
                         // Original is stored in OTItem pOriginalItem...
                         (OTItem::acceptFinalReceipt ==
                          pItem->GetType()) || // Accepting finalReceipt
                         //                       (OTItem::disputeFinalReceipt
                         // == pItem->GetType()) ||    // Disputing
                         // finalReceipt.
                         (OTItem::acceptBasketReceipt ==
                          pItem->GetType()) // Accepting basketReceipt
                         //                       (OTItem::disputeBasketReceipt
                         // == pItem->GetType())    // Disputing basketReceipt.
                         )) {
                        // The response item will contain a copy of the "accept"
                        // request.
                        ...
                        if (false == theInbox.LoadInbox()) {
                            OTLog::Error(
                                "Error loading inbox during processInbox\n");
                                   ...
                                   theInbox.VerifyAccount(m_nymServer)) {
                            OTLog::Error(
                        ...
                        // the case above, with receipts from cron.
                                  ...
                                  (OTItem::acceptPending ==
                                   pItem->GetType()) // acceptPending includes
                                                     ...
                                                     // they are
                                  //                                      (OTItem::rejectPending
                                  // == pItem->GetType())    // stored/loaded
                                  // similarly, not like the above Cron
                                  // Receipts.
                                  ) &&
                                 (NULL !=
                                   ...
                                   // a cron receipt.)
                        {
                            ...
                            // > 1)
                            //                                OTLog::vError("%s:
                            // WARNING 2: There are MULTIPLE receipts 'in
                            // reference to' %ld. (It will return the first
                            // one...)\n",
                            //                                              __FUNCTION__,
                            // pItem->GetReferenceToNum());
                            ...
                            if (NULL != pOriginalItem) {
                                ...
                                // TRANSFER
                                    ...
                                    //                                        bSuccessLoadingInbox
                                    // =
                                    // theFromInbox.GenerateLedger(IDFromAccount,
                                    // SERVER_ID, OTLedger::inbox, true); //
                                    // bGenerateFile=true
                        ...
                        OTLog::vError("OTServer::%s: Error, unexpected "
                                      "OTItem::itemType: %s\n",
    ...
    if (tranOut.GetSuccess()) {
        ...
        for (int32_t i = 0; i < theTempNym.GetIssuedNumCount(SERVER_ID); i++) {
            int64_t lTemp = theTempNym.GetIssuedNum(SERVER_ID, i);
            theNym.RemoveIssuedNum(m_nymServer, m_strServerID, lTemp,
...
// (like if it actually exists.)
{
    msgOut.m_strRequestNum.Set(
        theMessage.m_strRequestNum); // to prevent replay attacks.
  ...
  */
        ...
        ((OTServer::GetOverrideNymID().size() <=
         ...
         (0 != OTServer::GetOverrideNymID().compare(
                   (theMessage.m_strNymID.Get()))))) // match the Nym's ID who
                                                     // sent this message)
    {
        OTLog::vOutput(0, "OTServer::ProcessUserCommand: Nym %s: failed "
    ...
    if (false == ValidateServerIDfromUser(theMessage.m_strServerID)) {
        OTLog::Error("OTServer::ProcessUserCommand: Invalid server ID sent in "
                     "command request.\n");
        return false;
    }
    else {
    ...
    if (NULL == pNym)
        ...
        OTLog::vError(
            "OTServer::ProcessUserCommand: NymID on the optional Nym passed in "
            "(%s) "
    ...
    const bool bNymIsServerNym =
        (m_strServerUserID.Compare(theMessage.m_strNymID) ? true : false);
                    ...
                    {      // verified the Nym (including the credentials.)
                        ...
                        { // (Because we use authentication keys, not signing
                            ...
                            msgOut.m_strServerID =
                                m_strServerID; // ServerID, a hash of the server
                                               // contract.
                            msgOut.m_bSuccess = false;
                            ...
                            bool bLoadedSignedNymfile =
                                pNym->LoadSignedNymfile(m_nymServer);
    ...
    if (pNym->VerifyPseudonym()) {
        ...
        if (theMessage.VerifySignature(*pNym)) {
            ...
            if (bNymIsServerNym || pNym->LoadSignedNymfile(m_nymServer)) {
                ...
                if (false ==
                    pNym->GetCurrentRequestNum(m_strServerID, lRequestNumber)) {
                    ...
                    if (pNym->GetCurrentRequestNum(m_strServerID,
                                                   lRequestNumber)) {
                ...
                {
                    // pNym->IncrementRequestNum(m_strServerID); // commented
            ...
            numlist_ack_reply)) // returns false if the numlist was empty.
    {
        ...
        if (theNymbox.LoadNymbox() && theNymbox.VerifySignature(m_nymServer)) {
            FOR_EACH(std::set<int64_t>, numlist_ack_reply)
            {
                ...
                //
                if (false ==
                    pNym->VerifyAcknowledgedNum(m_strServerID, lRequestNum)) {
                    ...
                    if (NULL != pReplyNotice) {
                        ...
                        if (!bDeleted || !bRemoved)
                            OTLog::Error(
                                "OTServer::ProcessUserCommand: Failed trying "
...
{
    if (NULL == m_pServerContract) return false;
